...
const Youtube = ({arr}) => {
  ...
  useEffect(() => {
    ...
    fetch(apiCall).then((result) => result.json()).then((data) => {
        ...
        const count2 = data.items[0].statistics.viewCount;
        const count3 = data.items[0].statistics.videoCount;
        setSubscriberCount(func(count, 1));
        setViewsCount(func(count2, 1));
        setVideosCount(count3);
      });
  });
